            ...
            for csv_file in "${CSV_FILES[@]}"; do
              ./segment.sh \
                ...
                --window_size="$window_size" \
                  >> "$LOG_DIR/old/skip_window-${skip_window_option}_delete_too_close-${delete_too_close_option}_delete_low_variance-${delete_low_variance_option}sensor-${sensor}_kind-${segmentation_kind}_window-${window_size}.log" 2>&1
            done
          done
        done
      done
